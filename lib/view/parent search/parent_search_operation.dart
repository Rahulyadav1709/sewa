...
import 'dart:ui';
import 'package:sewa/view/parent%20search/parent_search_results.dart';

class ParentOperation extends StatefulWidget {
  const ParentOperation({super.key});

  @override
  State<ParentOperation> createState() => _ParentOperationState();
}

class _ParentOperationState extends State<ParentOperation>
  ...
  void initState() {
    ...
    // Fetch the Parent data when the widget initializes
    controller.getParentIdsFuture = controller.getParentInfoApi().then((value) {
      // Call _filterParent with an empty query after the data is loaded
      controller.filterParent('');
      return [];
    });
  }
          ...
          children: [
            ...
            Column(
              children: [
                ...
                Expanded(
                  child: FutureBuilder(
                    future: controller.getParentIdsFuture,
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    buildInfoSection(
                      'What is Parent?',
                      'Parent (Functional Parent) is a hierarchical system used to identify and organize physical Parents of equipment or assets within a facility.',
                    ),
                    const SizedBox(height: 16),
                    buildInfoSection(
                      'How to use',
                      'Use the search bar to find specific Parent IDs. Tap on any Parent to view detailed information and related equipment.',
                    ),
                    const SizedBox(height: 16),
                    buildInfoSection(
                      'Filtering',
      ...
      child: Column(
        ...
        children: [
          ...
          ElevatedButton.icon(
            onPressed: () {
              setState(() {
                controller.getParentIdsFuture = controller.getParentInfoApi();
              });
            },
          ...
          Expanded(
            child: TextField(
              ...
              decoration: InputDecoration(
                hintText: 'Search for Parent-Search...',
  ...
  Widget buildParentInfoCard() {
    return Obx(() {
      if (controller.filteredParentEntries.isEmpty) {
        return Center(
          child: Column(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
      ...
      return ListView.builder(
        ...
        itemCount: controller.filteredParentEntries.length,
        itemBuilder: (context, index) {
          final entry = controller.filteredParentEntries[index];
          return AnimatedBuilder(
            ...
            child: buildParentCard(entry, index),
          );
        },
      );
    });
  ...
  Widget buildParentCard(MapEntry<String, int> entry, int index) {
            ...
            PageRouteBuilder(
              pageBuilder:
                  (context, animation, secondaryAnimation) =>
                      ParentSearchResultScreen(
                        ParentID: entry.key.toUpperCase(),
                      ),
              transitionsBuilder: (
                context,
                animation,
